...
void initRunner(uint8_t x, uint8_t y)
{
    ...
    SET_VGA_SPR_ATTR (MAX_GUARDS, VGA_SPR_ATTR_EN | VGA_SPR_ATTR_SIZE_8 | VGA_SPR_ATTR_FRONT);
    SET_VGA_SPR_BUF_IDX (MAX_GUARDS, RUNNER_1>>4);
    SET_VGA_SPR_POS_X(MAX_GUARDS, xPos);  // x
    SET_VGA_SPR_POS_Y(MAX_GUARDS, yPos);  // y
...
void runnerMoveStep(uint8_t action, uint8_t stayCurrentPos)
{
    ...
    if (action == ACT_STOP) {
        ...
        seq  = runnerSequences[runner.sequence][runner.idx];
        SET_VGA_SPR_MIR   (MAX_GUARDS, dir);
        SET_VGA_SPR_POS_X (MAX_GUARDS, xPos);  // x
        SET_VGA_SPR_POS_Y (MAX_GUARDS, yPos);  // y
        SET_VGA_SPR_BUF_IDX (MAX_GUARDS, seq>>4);
        SET_VGA_SPR_SUB_IDX (MAX_GUARDS, seq);
...
void moveRunner()
{
    ...
    if (act == 0) {
      ...
      last   = act;
      if ((act == ACT_DIG_LEFT) || (act == ACT_DIG_RIGHT)) {
        repeat = 0;
      } else {
        repeat = 1;
      }
...
uint8_t getSeqOffset (uint8_t seq)
{
    return (seq & 3);
}