...
{
    //mxma    uint32_t spriteAttr0 = ((uint32_t)VERA_INC_1 << 16) | SPRITE_ATTR0;
    if (guardCount < MAX_GUARDS) {
        ...
        if (i < MAX_GUARDS) {
          SET_VGA_SPR_ATTR (i, VGA_SPR_ATTR_EN | VGA_SPR_ATTR_SIZE_8 | VGA_SPR_ATTR_FRONT);
          SET_VGA_SPR_BUF_IDX (i, GUARD_1>>4);
          SET_VGA_SPR_POS_X(i, xPos);  // x
          SET_VGA_SPR_POS_Y(i, yPos);  // y
        }
...
void guardMoveStep(uint8_t id, uint8_t action)
{
    ...
    if (action == ACT_STOP) {
        ...
        seq  = guardSequences[curGuard->sequence][curGuard->idx];
        SET_VGA_SPR_MIR (id, dir);
        SET_VGA_SPR_BUF_IDX (id, seq>>4);
        SET_VGA_SPR_SUB_IDX (id, seq);
        SET_VGA_SPR_POS_X(id, xPos);  // x
        SET_VGA_SPR_POS_Y(id, yPos);  // y
    ...
    for (i = 0; i < guardCount; i++) {
        if (shake[i].active) {
            ...
            if (shake[i].count >= shakeTimes[shake[i].idx]) {
                ...
                VGA_SPR_POSX(shake[i].id) = xPos;  // x
